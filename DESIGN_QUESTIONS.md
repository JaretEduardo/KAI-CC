...
✓ **(KAI LANGUAGE M10A)** A slice `[T]` is now a real semantic `TypeKind::Slice`, structurally distinct from Array: its identity is its element Type ALONE (no length - `SemanticModel::internSlice()`/`sliceElementType()`, a sibling interning table to Array's own, per the same compound-type architecture M7A established), so `[i32] == [i32]` regardless of runtime length, `[i32] != [u32]`, and `[i32] != [i32; 3]` (never interchangeable with, and never automatically converted to/from, a fixed array). A slice is a NON-OWNING, IMMUTABLE, runtime-length VIEW: `Slice<T> { ptr, len }` conceptually, not source-visible struct syntax; copying a slice (`let b = a`) copies only the view (ptr+len) - the underlying elements are never copied, and there is no refcount/allocation/copy-on-write, deliberately different from `[T; N]`'s own value-copy semantics. M10A introduces NO mutable-slice variant (`[mut T]`/`mut [T]`/`&mut [T]`) - element mutation through a slice remains unsupported until a future explicit design; a `mut` BINDING containing a slice may still be reassignable as a binding, which is unrelated to element mutability. A slice function parameter receives a copy of the view (pointer+length by value, no ownership transfer, no aliasing guarantee since multiple read-only views may share storage - safe because no mutation exists yet); slice RETURN values remain a DEFERRED lifetime problem - KAI has no borrow checker, so a slice viewing local storage must never be returned, but this is recorded as a design invariant only, not yet enforced by a dedicated checker. This is a TYPE-FOUNDATION-ONLY milestone: no LLVM lowering (`lowerType()` returns the same clean nullptr signal as every other unsupported Type), no array-to-slice conversion, no slice indexing, and no slice literals exist yet. See TYPE_SYSTEM.md's own "Slices" section and COMPILER_ARCHITECTURE.md's own M10A note.

## Open — KAI LANGUAGE M10B and beyond (slices)

The following were deliberately left OPEN by M10A (see TYPE_SYSTEM.md's own
"Slices" section) rather than decided silently:

- Exact user-facing array-to-slice conversion rule: is `[T; N]` -> `[T]`
  implicit in parameter position, an explicit builtin/operation, or explicit
  syntax? TYPE_SYSTEM.md's pre-M10A text states flatly "there is no
  array-to-slice conversion in KAI 0.1" - M10A does not change that; a future
  milestone must choose the actual mechanism before implementing it.
- Exact user-facing slice (and array) LENGTH operation. Audited: no `len(x)`/
  `x.len` builtin exists anywhere in the compiler today - `.len` appears only
  as illustrative, unimplemented example syntax for a future `String`/
  `Buffer<T>` in STANDARD_LIBRARY.md/GRAMMAR.md, member access itself is not
  yet semantically resolved (`checkMemberExpr()` always yields
  `Type::unresolved()`), and no other length/`sizeof`-style construct exists.
  Left open for M10B.
- Full slice-return lifetime/escape analysis: M10A only documents the "a
  slice must not escape the local storage it views" invariant (see
  TYPE_SYSTEM.md); no lifetime/borrow checker of any kind exists to enforce
  it yet, and designing one is out of M10A's scope.
- Future mutable-slice syntax/semantics (`[mut T]`, `&mut [T]`, or otherwise)
  - M10A deliberately ships immutable-slice-only.